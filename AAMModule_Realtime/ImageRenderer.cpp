...
HRESULT ImageRenderer::EnsureResources()
{
	...
	//	D2D1_SIZE_U size_render = D2D1::SizeU(m_sourceWidth*3, m_sourceHeight*3);
		...
		//d2d_brush_=new ID2D1SolidColorBrush(NULL);
		d2d_brush_->SetColor(D2D1::ColorF(
			0.0f,
			1.0f,
			0));
		showImgPts=new ShowImgPts(m_pRenderTarget,78);
    }
...
void ImageRenderer::setPtsInfo(float *_p,int _pNum)
{
	pts=_p;
	ptsNum=_pNum;
}
...
HRESULT ImageRenderer::Draw(BYTE* pImage, unsigned long cbImage)
{
	...
	showImgPts->drawImgPts(m_pBitmap,pts,ptsNum);
	hr = S_OK;
	...
	//		D2D1::Point2F(static_cast<FLOAT>(pts[i]-1),static_cast<FLOAT>(pts[i+ptsNum])), 
	//		D2D1::Point2F(static_cast<FLOAT>(pts[i]+ 1),static_cast<FLOAT>(pts[i+ptsNum] )), 
	//		d2d_brush_);
	//}
	...
	////{
	////	m_pRenderTarget->DrawLine(
	////		D2D1::Point2F(static_cast<FLOAT>(300-1),static_cast<FLOAT>(300)), 
	////		D2D1::Point2F(static_cast<FLOAT>(300+ 1),static_cast<FLOAT>(300 )), 
	////		d2d_brush_);
	////	/*m_pRenderTarget->DrawLine(
	////		D2D1::Point2F(static_cast<FLOAT>(300),static_cast<FLOAT>(300-1)), 
	////		D2D1::Point2F(static_cast<FLOAT>(300),static_cast<FLOAT>(300+1 )), 
	////		d2d_brush_);*/
	////}
 //           
...
HRESULT ImageRenderer::DrawImgOnly(BYTE* pImage, unsigned long cbImage)
{
    // incorrectly sized image data passed in
    if ( cbImage < ((m_sourceHeight - 1) * m_sourceStride) + (m_sourceWidth * 4) )
    {
