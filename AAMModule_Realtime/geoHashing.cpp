...
void GeoHashing::setupTotalVec()
{


	int totalKeySize=table->mappings.size();

	//cout<<"key size: "<<totalKeySize<<endl;
	//cout<<"basis number: "<<basisNum<<endl;

	totalVec=new Mat *[totalKeySize];

	int c_dim=2;
	int c_size[]={1,shapeNum};
	for (int i=0;i<totalKeySize;i++)
	{
		totalVec[i]=new Mat [basisNum];
		for (int j=0;j<basisNum;j++)
		{
			vector<int> *curLink=dataTabelVec[i]->at(j);
			//totalVec[i][j]=NULL;
			if (curLink!=NULL)
			{
				totalVec[i][j]=Mat::zeros(1,shapeNum,CV_32S);


				int curSize=curLink->size();
				if (curSize>0)
				{
					for (int k=0;k<curSize;k++)
					{
						totalVec[i][j].at<int>(0,curLink->at(k))=1;
					}
				}
			}

			/*if (dataTabelVec[i]->at(j)!=NULL)
			{
			cout<<dataTabelVec[i]->at(j)->size()<<" ";
			}*/

		}
		//cout<<endl;
		//totalVec[i]=Mat::zeros(basisNum,shapeNum,CV_16UC1);
	}
	...
	*/
	...
	for (int i=0;i<shapeNum;i++)
	{
		...
		for (int j=0;j<newXY.cols;j++)
		{
			...
			//int cid=iter->second;
			int cid=table->isInside(discretized);
...
#pragma omp parallel for
	for (int i=0;i<basisNum;i++)
	{
		...
		for (int j=0;j<newXY.cols;j++)
		{
			...
			int cind=table->isInside(discretized);
			if (cind<0)
			{
				continue;
			}
			...
			if (totalVec[cind][i].empty())
			{
				continue;
			}
			voteNum[i]+=totalVec[cind][i];
		}
	...
	/*for (int i=0;i<nnnum;i++)
	cout<<distanceVec[resortVec[i].second].second<<" ";
	cout<<endl;*/
							...
							vector<Point2f>*candidatePts)
{
	...
	/*Mat curImg=imread("D:\\Fuhao\\face dataset\\kinect sequences\\Zain Expression\\Zain Expression_10298.jpg");
	for (int i=0;i<ptsNum;i++)
	{
	circle(curImg,Point(tmp.at<double>(0,i),tmp.at<double>(0,ptsNum+i)),2,Scalar(255));
	}
	imshow("1",curImg);
	waitKey();*/
	...
	//gCodeTimer.printTimeTree();
	//double time = total_fps;
	//cout<<"inlier ransac: "<<time<<" ms"<<endl;
